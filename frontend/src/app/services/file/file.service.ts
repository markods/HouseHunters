...
export class FileService {
  ...
  async add( file: FileData ): Promise<[ Status, ObjectId?/*file_id*/ ]>
  {
    try
    {
      ...
      let res = await this.http.post( `${this.url}/file/add`, file ).toPromise() as [ Status, ObjectId? ];
      return res;
    }
    catch( err )
    {
  ...
  async get( file_id: ObjectId ): Promise<[ Status, FileData? ]>
  {
    try
    {
      ...
      let res = await this.http.post( `${this.url}/file/get`, file_id ).toPromise() as [ Status, FileData? ];
      return res;
    }
    catch( err )
    {
